...
from rebuttal_phase.TAGConv import QTopologyAdaptiveGraphConvolution, MQTopologyAdaptiveGraphConvolution


class MQTAG(Module):
    def __init__(self, num_channels: int, hidden_channels: int, out_channels: int, num_bits: List[int], k: int = 3):
        super(MQTAG, self).__init__()
        self.tag_1 = MQTopologyAdaptiveGraphConvolution(in_channels=num_channels,
        ...
        self.tag_2 = MQTopologyAdaptiveGraphConvolution(in_channels=hidden_channels,
    ...
    def reset_parameters(self):
        self.tag_1.reset_parameters()
        self.relu_1.reset_parameters()
        self.tag_2.reset_parameters()

    def forward(self, x, edge_index, edge_attr):
        x = self.tag_1(x, edge_index, edge_attr)
        x = self.relu_1(x)
        x = dropout(x, training=self.training)
        x = self.tag_2(x, edge_index, edge_attr)
        return x

    def calculate_loss(self, x, edge_index, edge_attr):
        loss = 0
        loss += self.tag_1.calculate_weighted_loss(x, edge_index, edge_attr)
        x = self.tag_1(x, edge_index, edge_attr)
        loss += self.relu_1.calculate_weighted_loss(x)
        x = self.relu_1(x)
        loss += self.tag_2.calculate_weighted_loss(x, edge_index, edge_attr)
        return loss

    @no_grad()
    def estimated_bit_operation_precision(self, x, edge_index, edge_attr):
        bit_operations = 0
        bit_operations += self.tag_1.estimated_bit_operation_precision(x, edge_index, edge_attr)
        x = self.tag_1(x, edge_index, edge_attr)
        bit_operations += self.relu_1.estimated_bit_operation_precision(x)
        x = self.relu_1(x)
        bit_operations += self.tag_2.estimated_bit_operation_precision(x, edge_index, edge_attr)
...
class QGCN(Module):
    def __init__(self, num_channels: int, hidden_channels: int, out_channels: int, num_bits: List[int], k:int = 3):
        ...
        self.tag_1 = QTopologyAdaptiveGraphConvolution(in_channels=num_channels,
                                                       out_channels=hidden_channels,
                                                       k=k,
                                                       qi=True,
                                                       qo=True,
        ...
        self.tag_2 = QTopologyAdaptiveGraphConvolution(in_channels=hidden_channels,
                                                       out_channels=out_channels,
                                                       k=k,
                                                       qi=False,
                                                       qo=True,
    ...
    def simulated_quantize_forward(self, x, edge_index, edge_attr):
        x = self.tag_1.simulated_quantize_forward(x, edge_index, edge_attr)
        x = self.relu_1.simulated_quantize_forward(x)
        x = dropout(x, training=self.training)
        x = self.tag_2.simulated_quantize_forward(x, edge_index, edge_attr)
        return x

    def estimated_bit_operation_precision(self, x, edge_index, edge_attr):
        gcn_1_operations = self.tag_1.estimated_bit_operation_precision(x, edge_index, edge_attr)
        x = self.tag_1(x, edge_index, edge_attr)
        relu_bit_operations = self.relu_1.estimated_bit_operation_precision(x)
        x = self.relu_1(x)
        gcn_2_operations = self.tag_2.estimated_bit_operation_precision(x, edge_index, edge_attr)
...
if __name__ == '__main__':
    ...
    for run_i in range(args.num_runs):
        model = MQTAG(dataset.num_features, args.hidden_channels, dataset.num_classes, args.num_bits_list)
        model.to(device)
        ...
        wining_bit_width = [sum(model.tag_1.select_top_k_winners(1).values(), []),
                            sum(model.relu_1.select_top_k_winners(1).values(), []),
                            sum(model.tag_2.select_top_k_winners(1).values(), []),
                            ]
