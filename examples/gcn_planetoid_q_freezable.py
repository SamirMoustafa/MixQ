...
class QGCN(Module):
    ...
    def freeze(self):
        self.gcn_1.freeze()
        self.relu_1.freeze(qi=self.gcn_1.qo)
        self.gcn_2.freeze(qi=self.gcn_1.qo)
    ...
    def quantize_inference(self, x, edge_index, edge_attr):
        ...
        qx = self.relu_1.quantize_inference(qx)
        qx = self.gcn_2.quantize_inference(qx, edge_index, edge_attr)
        x = self.gcn_2.qo.dequantize(qx)
        return x
...
if __name__ == '__main__':
    num_runs = 10
    bit_width = [[8, 8, 8, 8, 8, 8],  # Layer 1 bit-widths (Linear input, Linear output, Linear weight), Edge weight, (Message passing input, Message passing output, Message passing weight)
                 [8],  # Layer 2 bit-widths input
                 [-1, 8, 8, 8, 8, 8],  # Layer 2 bit-widths (Linear input, Linear output, Linear weight), Edge weight, (Message passing input, Message passing output, Message passing weight)
                 ]
    epochs = 200
    lr = 0.001
    lr_quant = 0.0001
    ...
    fp32_accuracies, simulated_quantize_accuracies, quantize_inference_accuracies = [], [], []
    for run_i in range(num_runs):
        ...
        print(f"[{run_i + 1}/{num_runs}]: Quantized Inference Model, Accuracy: {test_accuracy:.2f}%")
        quantize_inference_accuracies.append(test_accuracy)

    print("=" * 80)
    print(f"FP32 Model, Accuracy: {np.mean(fp32_accuracies):.2f}% ± {np.std(fp32_accuracies):.2f}%")
    print(f"Simulated Quantized Model, Accuracy: {np.mean(simulated_quantize_accuracies):.2f}% ± {np.std(simulated_quantize_accuracies):.2f}%")
    print(f"Quantized Inference Model, Accuracy: {np.mean(quantize_inference_accuracies):.2f}% ± {np.std(quantize_inference_accuracies):.2f}%")
