...
class QGCN(Module):
    def __init__(self, num_channels: int,
        ...
        for i in range(number_of_layers):
            is_first_layer = i == 0
            is_last_layer = i == number_of_layers - 1
            gcn_i = QGraphConvolution(in_channels=hidden_channels if not is_first_layer else num_channels,
                                      out_channels=hidden_channels if not is_last_layer else out_channels,
                                      qi=is_first_layer,
                                      qo=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bit_width_search_space", type=int, nargs="+", default=[4, 8])
    ...
    dataset = PygNodePropPredDataset(name=args.dataset_name, root="../../data/", transform=ToUndirected())
    ...
    best_epoch, best_state_dict, best_valid_loss, final_test_acc = 0, None, float("inf"), 0
    ...
    for run in range(args.num_runs):
        ...
        best_epoch, best_valid_acc, final_test_acc = 0, 0, 0
        pbar = tqdm(range(args.epochs))
        for epoch in pbar:
            loss = train(model, data, train_idx, optimizer, lr_scheduler, criterion)
            accs, losses = evaluate(model, data, split_idx, evaluator, criterion)
